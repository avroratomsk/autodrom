(() => {
    ...
    if (linkCatalog.length > 0) linkCatalog.forEach((link => {
    ...
    if (menu__btn) menu__btn.addEventListener("click", (function(e) {
        if (e.target.classList.contains("menu__btn") || e.target.closest(".menu__btn")) {
            this.classList.toggle("_active");
            if (menu_catalog) menu_catalog.classList.toggle("_active");
        }
    }));
    window["FLS"] = true;
    isWebp();
